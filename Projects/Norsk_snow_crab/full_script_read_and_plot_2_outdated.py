...
def main():
    ...
    for group, clusters in geofence_results.items():
        ...
        for cluster_id, results in clusters.items():
            cluster_layer = FeatureGroupSubGroup(parent_layer, name=f"{group} – Cluster {cluster_id}", show=True)
            m.add_child(cluster_layer)

            # Points
            points_layer = FeatureGroupSubGroup(cluster_layer, name=f"Cluster {cluster_id} points", show=True)
            ...
            # Convex hull
            convex_layer = FeatureGroupSubGroup(cluster_layer, name=f"Cluster {cluster_id} convex", show=False)
            ...
            # Concave hull
            concave_layer = FeatureGroupSubGroup(cluster_layer, name=f"Cluster {cluster_id} concave (alpha={results['alpha']:.5f})", show=True)
            ...
            # Buffer
            buffer_layer = FeatureGroupSubGroup(cluster_layer, name=f"Cluster {cluster_id} buffer (+5 km)", show=False)
    ...
    m.save("geofence_map_what2.html")
